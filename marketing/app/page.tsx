      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-slate-50 to-white pt-20 pb-24 px-6 text-center">
        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-slate-900 mb-5 leading-tight">
            Know what your team
            <br className="hidden sm:block" /> is working on
          </h1>

          <p className="text-lg text-slate-500 max-w-xl mx-auto mb-8 leading-relaxed">
            Automatic screenshots, multi-monitor support, and a built-in
            employee review window — deployed in under 10 minutes.
          </p>
              ...
              <div className="flex-1 p-4">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {Array.from({ length: 8 }).map((_, i) => (
                    <div
                      key={i}
                      className="aspect-video rounded-lg bg-gradient-to-br from-slate-200 to-slate-300"
                    />
                  ))}
                </div>
              </div>
                ...
                <ul className="flex-1 space-y-2.5 mb-6">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-start gap-2 text-sm">
                      ...
                      />
                      <span
                        className={plan.highlight ? "text-brand-100" : "text-slate-600"}
                      >
                        {f}
                      </span>
                    </li>
      ...
      <footer className="border-t border-slate-100 bg-white py-8 px-6">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex gap-4 text-xs text-slate-400">
            <a href={`${APP_URL}/login`} className="hover:text-slate-600">
              Sign in
            </a>
